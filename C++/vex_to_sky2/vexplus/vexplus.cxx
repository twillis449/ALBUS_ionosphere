...
// Taken over: 2005 Jun 08 by James M Anderson for Ionosphere work
...
static char rcsid[]="@(#)$Id: vexplus.C,v 3.4 2005/06/07 12:56:02 olnon Exp $";
...
string VexPlus::PIName() const {

  void* ptr;  
  int   islink, isname;
  char *value, *units;

  if( !(ptr = get_global_lowl(T_PI_NAME,B_EXPER,vexp)) ) 
    return string("");
  if( (vex_field(T_PI_NAME, ptr, 1,
		 &islink,&isname,&value,&units)) ) 
    return string("");
...
//------------------------------------------------------------------------
// Method:        N_Scans
// Description:   Number of scans in VEX file
// Input:         
// Output:        
// Return value:  0, no scans, default
//                > 0 number of scans
//               
// Side effects: 
//------------------------------------------------------------------------
/*
int VexPlus::N_Scans( ) const {

  int icount = 0;

  if( char* ptr = get_mode_def( vexp ) ) {
    do {
      icount++;
    } while ( (ptr = get_mode_def_next()) );
  }
...
// Side effects: [[SIMPLIFICATION]] Fails for fan-in
//------------------------------------------------------------------------
int VexPlus::N_Bits( const string& stat,
		     const string& mode ) const {
  ...
  if( (ptr = get_all_lowl(stat_c,mode_c,T_FANOUT_DEF,B_TRACKS,vexp)) ) {
    do {
      vex_field(T_FANOUT_DEF, ptr, 3, 
		&islink,&isname,&value,&units);
      if( (strcmp(value,"sign")) && (strcmp(value,"mag")) ) return 0;
      else {
...
// Side effects: 
//------------------------------------------------------------------------
int VexPlus::FanOut( const string& stat,
		     const string& mode ) const {
  ...
  if( !(ptr = get_all_lowl(stat_c,mode_c,T_FANOUT_DEF,B_TRACKS,vexp)) )
    return 0;
  if( (vex_field(T_FANOUT_DEF, ptr, 5, 
		       &islink,&isname,&value,&units)) ) 
    return 0;

  //at least one track if next fails return 1
  if( (vex_field(T_FANOUT_DEF, ptr, 6, 
		       &islink,&isname,&value,&units)) ) 
    return 1;

  //at least two tracks
  if( (vex_field(T_FANOUT_DEF, ptr, 7, 
		 &islink,&isname,&value,&units)) )
    return 2;
  
  // there are 3 than there must be 4, so this must work
  if( (vex_field(T_FANOUT_DEF, ptr, 8, 
		 &islink,&isname,&value,&units)) )
    return 0;
...
// Side effects: 
//------------------------------------------------------------------------
string VexPlus::Track( const string& stat, const string& mode, 
		       const int& icount ) const {
  ...
  for( int iline = 0; iline < n_lines; iline++ ) {
    ...
    for( int ifan = 0; ifan < n_fan; ifan++ ) {
      ...
      if( (vex_field(T_FANOUT_DEF, ptr, 5+ifan,
		     &islink,&isname,&value,&units)) ) 
	return string("");
...
int VexPlus::TrackNr( const string& stat, const string& mode,
  ...
  //
  if( (vex_field(T_FANOUT_DEF, ptr, fieldNr, 
		 &islink,&isname,&value,&units)) ) 
    return 0;
