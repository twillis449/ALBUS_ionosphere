...
c----------------------------------------------------------------
     ...
     &     /ARGEXP/ARGMAX
      logical 	f1reg              
C     JMA  --as far as I can tell, this IRI code was not designed
C     to be used above 1000 km.  Add a bunch of checks to make sure
C     we don't blow up
      IF(HMF2 .lt. 900.) THEN
         DXDH = (1000.-HMF2)/700.
      ELSE
      ...
      IF(H.lt.1000.) THEN
         xmx0 = (H-HMF2)/DXDH
      ELSE
C         print *,H,HMF2,BETA,ETA,ZETA,DELTA,XNMF2
         xmx0 = (1000.-HMF2)/DXDH
      ENDIF
      x = xmx0 + x0
      ...
      IF(H.ge.1000.) THEN
         y = y + BETA * ETA * (H-1000.)/DXDH / beta
      ENDIF
      
      Y = y * dxdh
C     JMA  --let's have something to drop this down
C     a bit as an exponential at high altitudes
      IF(H.gt.1000.) THEN
         Y = Y + (H-1000.)/ (700.*sqrt(H*0.0007))
      ENDIF
C	JMA  Hey, the following does not look good.  If Y is bad, then
C     don't let it blow up.
C        if(abs(Y).gt.argmax) Y = sign(argmax,Y)
      if(abs(Y).gt.argmax) THEN
         XE1 = 0.0
         return
...
C        print*,H,HME,XNME,HEF,HMD,XNMD,HDX
...
C --------------------------------------------------------- TRANSITION
        COMMON/ARGEXP/ARGMAX
      COMMON /ARGEXP2/ARGMAX2
C     JMA  --FORTRAN does not have log1p(x), so will have to do this
C     by hand.  \ln(1+z) = z-1/2 z^2+1/3 z^3-... for |z| \leq 1 and z\neq 1
        D1 = ( X - HX ) / SC
C     The original code was
C        IF (ABS(D1).LT.ARGMAX2) GOTO 1
C     Which has be be modified, as D1 > 0 is different from D1 < 0
        IF(D1.GT.0.0) THEN
           IF(D1.LT.ARGMAX2) THEN
              GOTO 1
           ELSE 
              EPTR = D1
           ENDIF
        ELSE
           IF(D1.GT.-0.6*ARGMAX2) THEN
              GOTO 1
           ELSEIF(D1.LT.-ARGMAX) THEN
              EPTR = 0.0
           ELSE
C     The original code was
C          EPTR = 0.0
C     Which is BS.  Using the series expansion, I have
              EPTR = EXP( D1 )
              EPTR = EPTR-0.5*EPTR*EPTR
           ENDIF
        ENDIF
        RETURN
...
c
c
      subroutine tcon(BPATH,yr,mm,day,idn,rz,ig,rsn,nmonth)
c----------------------------------------------------------------
c input:        BPATH           base path to files
C               yr,mm,day       year(yyyy),month(mm),day(dd)
...
c----------------------------------------------------------------
      ...
      CHARACTER*256 BPATH, FILENA, stupid_FORTRAN_STRINGS
...
c
      if(iflag.eq.0) then
         ...
         FILENA = stupid_FORTRAN_STRINGS(BPATH,'ig_rz.dat')
         open(unit=12,file=FILENA,status='old')
...
c Read all the ionoindx and indrz values
 ...
 1       continue
      ...
      if (iytmp .lt. iymst .or. iytmp .gt. iymend) then
         if(konsol.ne.6) write(konsol,8000) iytmp,iymst,
     &        iymend
...
C*****************************************************************
 ...
 002    B1(I,J,M)=Y6(N1)
        ...
        IF(konsol.ne.6)
     &          WRITE(konsol,*)'LSTID are for AE>500. and ABS(FI)<65.'
        GOTO 004
        ENDIF
        TS=TS70+(-1.5571*FI+109.)/60.
        IF(TS.LT.SUX.AND.TS.GT.SAX)THEN
        IF(konsol.ne.6)
     &          WRITE(konsol,*)' LSTID are only at night'
        GOTO 004
        ENDIF
...
c
        ...
        SUBROUTINE APF(BPATH,IYYYY,IMN,ID,HOUR,IAP)
c--------------------------------------------------------------------
...
c--------------------------------------------------------------------
        ...
        FILENA = stupid_FORTRAN_STRINGS(BPATH,'ap.dat')
        Open(13,FILe=FILENA,
...
C      OPEN(13,FILE='/usr/local/etc/httpd/cgi-bin/models/IRI/ap.dat',
     *    ACCESS='DIRECT',RECL=39,FORM='FORMATTED',STATUS='OLD')
        ...
        if(iyyyy.gt.2005) goto 21
        if((iyyyy.eq.2005).and.(imn.gt.8)) goto 21
        is=0
...
c ap.dat file starts at January 1, 1960
  ...
  10    FORMAT(3I3,8I3,F5.1)
        goto 20
21      if(konsol.ne.6) write(konsol,100)
...
C      CALLING THE PROGRAM TO CONVERT TO GEOMAGNETIC COORDINATES
       ...
       ELSE
          IF(konsol.ne.6) THEN
             ...
             WRITE (konsol,*)' '
             WRITE (konsol,*)
     &            '   Wrong Coordinates Selection -------- >>', coor
             WRITE (konsol,*)' '
          ENDIF
          GOTO 100
       ENDIF
     ...
     &           '      Wrong Day of Year value --- >>', doy
      ...
      if(rgma.gt.90.0.or.rgma.lt.-90.0)then
         IF(konsol.ne.6) THEN
            ...
            WRITE (konsol,*)' '
            WRITE (konsol,*)
     &           '   Wrong GEOMAGNETIC LATITUDE value --- >>', rgma
            WRITE (konsol,*)' '
         ENDIF
          GOTO 100
      end if
...
100   CONTINUE
      ...
      CHARACTER*256 FUNCTION stupid_FORTRAN_STRINGS(PATH,FILE)
C      CHARACTER*256 stupid_FORTRAN_STRINGS
      CHARACTER*256 PATH
      CHARACTER*(*) FILE
      INTEGER I
      stupid_FORTRAN_STRINGS = PATH
      i = 257
 10   CONTINUE
      i = i - 1
      if(i.lt.1) GOTO 20
      IF(stupid_FORTRAN_STRINGS(i:i).eq.' ') GOTO 10
      i = i+1
      if(i.gt.256) GOTO 20
      stupid_FORTRAN_STRINGS(i:i) = '/'
      i = i+1
      if(i.gt.256) GOTO 20
      stupid_FORTRAN_STRINGS(i:) = FILE
 20   CONTINUE
      RETURN
      END
