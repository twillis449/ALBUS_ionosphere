...
C
C
       SUBROUTINE IRI_SUB(BPATH,JF,JMAG,ALATI,ALONG,IYYYY,MMDD,DHOUR,
...
c
     ...
     &          'obtained with Gulyaeva-1987 model.'
        if(OLD79) write(konsol,*) 'Ne: No F10.7-saturation for NmF2.',
     ...
     &       write(konsol,*) 'Ne, hmF1: provided by user.'
...
9427    if(konsol.ne.6) WRITE(KONSOL,11) 
...
c Te at fixed heights from IK, Truhlik, Triskova, Smilauer
              ...
              do 3491 ijk=4,7
                 call igrf_sub(BPATH,lati,longi,ryear,ahh(ijk),
     &                xl,icode,dipl,babs)
                 if(xl.gt.10.) xl=10.
...
c-----------------------------------------------------------------------        
c input:   BPATH         base pathname to datafiles
C          jmag,alati,along,iyyyy,mmdd,dhour  see IRI_SUB
...
1249            oarr(i)=oar(i) 
            xhour=dhour+iut*25.
            call IRI_SUB(BPATH,JF,JMAG,ALATI,ALONG,IYYYY,MMDD,XHOUR,
...
1349                    oarr(iii)=b(iii,i) 
                call IRI_SUB(BPATH,JF,JMAG,ALATI,ALONG,IYYYY,MMDD,DHOUR,
