...
C
C
       SUBROUTINE IRI_SUB_NE(BPATH,JF,JMAG,ALATI,ALONG,IYYYY,MMDD,DHOUR,
     &    HEIBEG,HEIEND,HEISTP,OUTF,OARR)
C-----------------------------------------------------------------
C
C INPUT:  BPATH         base pathname to datafiles
C         JF(1:30)      true/false switches for several options
...
C*****************************************************************
     ...
     &      NMF1,NME,NMD,MM,MLAT,MLONG,NOBO2,TI1
      CHARACTER*256  BPATH,stupid_FORTRAN_STRINGS
      CHARACTER*256  FILNAM
      REAL HEF,HF1, HMF2,Xe2H,XE2,HMF1
      REAL HF2,XF1,XF2,XE3_1,HST
      REAL EPS
...
c
      ...
      if(icalls.gt.1.or.konsol.ne.6) goto 8201
     ...
     &          ' of equatorial topside is not included.'
        if(FOF2IN) then
                write(konsol,*) 'Ne, foF2/NmF2: provided by user.'
                goto 2889
                endif
        if(URSIF2) then
        ...
        if(HMF2IN) write(konsol,*) 'Ne, hmF2/M3000F2: provided by user.'
        if(fof1in) write(konsol,*) 'Ne, foF1/NmF1: provided by user.'
        if(HMF1IN.and.LAYVER) write(konsol,*) 
     &       'Ne, hmF1: provided by user.'
        if(foein) write(konsol,*) 'Ne, foE/NmE: provided by user.'
        if(HMEIN) write(konsol,*) 'Ne, hmE: provided by user.'
      if(F1_OCPRO) write(konsol,*) 
     &       'Ne, foF1: probability function used.'
      if(F1_L_COND) write(konsol,*) 
     &     'Ne, foF1: L condition cases included.'
      if(DREG) then 
            write(konsol,*) 'Ne, D: Mechtley/Bilitza'
      else
            write(konsol,*) 'Ne, D: 3 models (M&B,F&T,Da)'
      endif
      if(jf(26)) then
            if(fof2in) then 
                       write(konsol,*) 
     &'Ne, foF2: Storm model turned off if foF2 or NmF2 user input'
                  jf(26)=.false.
                       ...
                       write(konsol,*) 'Ne, foF2: storm model included'
            endif
      endif
2889    continue
        if((.not.NOION).and.(DY)) 
     &    write(konsol,*) 'Ion Com.: Using Danilov et al. 1985/95.'
        if(NOTEM) goto 8201
      if(TENEOP)
     &    write(konsol,*) 'Te: Temperature-density correlation is used.'
      if(jf(23)) then 
            write(konsol,*) 'Te: Aeros/ISIS model'
      else
            write(konsol,*) 'Te: Interkosmos model'
      endif
8201    continue
...
c
      ...
      IF(DEPTH.LT.1.0) GOTO 600
        ...
        IF(.NOT.EXT) GOTO 667
        if(konsol.ne.6) WRITE(KONSOL,650)
...
9245    XE2H=XE2(HEF)
        if(xe2h.gt.bnmf1) then
         ...
         EPS=0.001
         CALL REGFA1(HEF,HMF2,XE2H,NMF2,EPS,NMF1,XE2,SCHALT,HMF1)
        IF(.not.SCHALT) GOTO 3801
c
c omit F1 feature ....................................................
c
9427    if(konsol.ne.6) then
            WRITE(KONSOL,11) 
...
380     continue
      ...
      EPS=0.001
      CALL REGFA1(hf1,HF2,XF1,XF2,EPS,NME,XE3_1,SCHALT,HST)
      if(schalt) goto 3885
...
c
3885  if(konsol.ne.6) WRITE(KONSOL,100)
100   FORMAT(1X,'*NE* HST IS NOT EVALUATED BY THE FUNCTION XE3')
      HZ=(HEF+HF1)/2.
      xnehz=xe3_1(hz)
        if(konsol.ne.6) WRITE(KONSOL,901) HZ,HEF
...
6153    IF(HMF1IN) THEN
     ...
     &                  HV1R,HV2R,HHALF,HXL,SCL,AMP,IIQU)
        IF((IIQU.EQ.1).and.(konsol.ne.6)) WRITE(KONSOL,7733)
7733    FORMAT('*NE* LAY amplitudes found with 2nd choice of HXL(1).')
        IF((IIQU.EQ.2).and.(konsol.ne.6)) WRITE(KONSOL,7722)
...
c-----------------------------------------------------------------------        
c input:   BPATH         base pathname to datafiles
C          jmag,alati,along,iyyyy,mmdd,dhour  see IRI_SUB_NE
...
1249            oarr(i)=oar(i) 
            xhour=dhour+iut*25.
            call IRI_SUB_NE(BPATH,
     &           JF,JMAG,ALATI,ALONG,IYYYY,MMDD,XHOUR,
...
1349                    oarr(iii)=b(iii,i) 
                call IRI_SUB_NE(BPATH,
     &                       JF,JMAG,ALATI,ALONG,IYYYY,MMDD,DHOUR,
