...
C
      subroutine igrf_sub(BPATH,xlat,xlong,year,height,
     &          xl,icode,dipl,babs)
c-----------------------------------------------------------------------        
c INPUT:
C    BPATH     base path to database files
...
C
C
        SUBROUTINE FELDCOF(BPATH,YEAR,DIMO)
c-----------------------------------------------------------------------        
...
c-----------------------------------------------------------------------        
        ...
        CHARACTER*256    FILMOD, FIL1, FIL2           
...
C-- DETERMINE IGRF-YEARS FOR INPUT-YEAR
        ...
        DTE1 = DTEMOD(L)   
        FIL1 = stupid_FORTRAN_STRINGS(BPATH,FILMOD(L))
        DTE2 = DTEMOD(L+1) 
        FIL2 = stupid_FORTRAN_STRINGS(BPATH,FILMOD(L+1))
