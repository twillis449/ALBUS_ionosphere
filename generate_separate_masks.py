...
class make_separate_masks:
  def __init__(self,hdu,file_name) :
    self.out_data = {}
    self.hdu = hdu
    self.mask = check_array(self.hdu.data)
    self.file_name = file_name
    self.residual_mask = self.mask 
    self.press=False
    self.move = False
...
# print('starting plot')
    ...
    if end_point > -1:
      self.image_title = self.file_name[:end_point] + ' generate residuals mask'
    else:
      self.image_title = self.file_name + ' generate resuiduals mask'
    ...
    norm = vis.ImageNormalize(vmin=vmin, vmax=vmax, stretch=vis.LogStretch(1000))
    self.ax.imshow(self.residual_mask, cmap =plt.cm.gray_r, norm = norm, origin = 'lower') 
    self.ax.scatter(cen_x-1, cen_y-1, s=40, marker='+')
    cs = self.ax.contour(self.mask, levels, linewidths = 1.0)
    self.ax.set_title('Original Mask Contours')
...
#   print ('info',hdu_list.info())
   hdu = hdu_list[0]
   print('hdu',hdu)
   make_separate_masks(hdu,filename)
