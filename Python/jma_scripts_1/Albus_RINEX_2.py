...
_DATA_POS_LIST = ['C1','P1','P2','L1','L2','S1','S2','SF1','SF2','LL',
                  'STECP', 'SIGMA', 'STECL', 'EL', 'AZ', 'STECPL']
_DATA_POS = {}
_DATA_POS_SIZE = len(_DATA_POS_LIST)
for i in xrange(_DATA_POS_SIZE): _DATA_POS[_DATA_POS_LIST[i]] = i
...
numarrays, should propagate back to the caller


INPUTS:
step_max   I  Maximum number of steps in time to look away
Sat_array  I  numarray of satellite data positions, as
...
"""
    ...
    for i in xrange(NUM_TIMES):
        if(Sat_array[i,:].sum() == -MAX_POSSIBLE_SATELLITES):
            ...
            sat_pos = 0
            for s in xrange(MAX_POSSIBLE_SATELLITES):
                ...
                for p in xrange(_DATA_POS_SIZE):
...
See the RIENX manuals at  ftp://igscb.jpl.nasa.gov/pub/data/format
...
MJD_check  I  Start date as MJD of data.  If None, then any date is allowed.
              ...
              0.75 days.  If not valid, raise a RuntimeError
...
One_Day_Limit I Should the RINEX data be limited to one day?  If 0, then no, and
              allow all data fromt he fiel to be read.  If not 0, then limit
...
MJD        O  numarray of Modified Julian Dates for each observation time
              as MJD[num_times]
Sat_array  O  numarray of satellite data positions, as
              Sat_array[num_times,MAX_POSSIBLE_SATELLITES]
              If the value is -1, then no observation of that satellite was made
...
"""
    ...
    try:
        ...
        if((line[60:80] != "RINEX VERSION / TYPE")
           or (line[20] != 'O')):
            raise RuntimeError("Error: RINEX file '%s' not a proper observation file"%filename)
        version = float(line[0:9])
        # int() truncates!
        if((int(version) != 2) or (version > 2.11)):
            raise RuntimeError, "Error: RINEX file '%s' has unsupporter version type %.2f"%(filename, version)
        while(1):
            line = fp.readline()
            if(line[60:73] == "END OF HEADER"):
                break
            elif(line[60:79] == "APPROX POSITION XYZ"):
                XYZ = numarray.zeros((6), type='Float64')
                XYZ[0:3] = map(float,line.strip().split(None,3)[0:3])
                ...
                offset = map(float,line.strip().split(None,3)[0:3])
                if(XYZ == None):
                ...
                ymdhms = map(int,line.strip().split(None,5)[0:5])
                ymdhms.append(float(line[30:43]))
                if(MJD_check is not None):
                    MJD_start=jma_aips.get_MJD_hms(ymdhms[0],ymdhms[1],ymdhms[2],
                    ...
                    MJD_start = jma_aips.get_MJD_hms(ymdhms[0],ymdhms[1],
                                                     ymdhms[2],
                                                     ymdhms[3],ymdhms[4],
                                                     ymdhms[5])
                ...
                while(count < num_data):
                    ...
                    if(code in _DATA_POS.keys()):
                        obs_info[count] = _DATA_POS[code]
                    elif((code[0] == 'P') or (code[0] == 'L') or (code[0] == 'S')
                         or ((code[0] == 'C') and(code[1] != '2'))):
                        raise RuntimeError("Error: RINEX file '%s' contains unsupported data.  Ask provider to update code to support %s"(filename, code))
                    else:
                        pass
                    count += 1
        # Now check that we got everything necessary
        if((num_data == None) or (ymdhms == None) or (XYZ == None)):
        ...
        if(time_offset == None):
            if(time_system == 'GPS'):
                ...
                raise RuntimeError("Unsupported Time system in file '%s'"%filename)
        # Read in the data
        if(num_times > 0):
            MJD = numarray.zeros((num_times), type='Float64')
            Sat_array = numarray.zeros((num_times,MAX_POSSIBLE_SATELLITES),
                                       type='Int16')
            obs_data = numarray.zeros((num_times, max_sat, _DATA_POS_SIZE),
                                      type='Float64')
            Sat_array -= 1
        ...
        while(1):
                ...
                if(epoch_flag > 1):
                    ...
                    if(len(line) >= 32):
                        ...
                        try:
                            ...
                            continue
                    for skip in xrange(num_list_to_skip):
                        line = fp.readline()
                        if(PRINT_DEBUG_LEVEL > 0):
                            sys.stderr.write(line)
                    continue
            ...
            if(num_times > 0):
                ...
                MJD_now = jma_aips.get_MJD_hms(year, month, day,
                                               hour, minute, second)
            ...
            if(num_sat > max_sat):
                if(num_times > 0):
                    raise RuntimeError("Function called with too little satellite space")
                max_sat = num_sat
            line_pos = 32-3
            sat_count = 0
            SBAS_warned = 0
            while(sat_count < num_sat):
                ...
                if(num_times > 0):
                    ...
                    if((code == ' ') or (code == 'G')):
                        ...
                        raise RuntimeError("Unsupported satellite type '%s' in observation at line '%s'"%(code,line))
                    assert(sat >= 0)
            ...
            if((num_times < 0) or (time_index >= num_times)):
                ...
                for i in xrange(num_sat*lines_per_sat):
                    line = fp.readline()
            else:
                for sat in xrange(num_sat):
                    line = fp.readline()
                    l = len(line)
                    line_pos = -16
                    obs_count = 0
                            ...
                            if((line[line_pos+15].isspace()) or
                               (line[line_pos+15] == '0')):
                                ...
                                if(freq == 1):
                                    ...
                                    raise RuntimeError("Unsupported Frequency code in '%s'"%obs_info_code[count])
                        obs_count += 1
    ...
    if(num_times < 0):
        if(One_Day_Limit == 0):
            MJD_end = jma_aips.get_MJD(year, month, day) +1.0
...
"""
    ...
    if((sat >= 0)and(sat <= 99)):
        ...
        # GPS
        raise RuntimeError("Galileo orbits unkown")
    else:
        raise RuntimeError("Unknown satellite %d"%sat)
    i_0 = 0.0
    r_0 = sat_r.mean()
    ...
    t_01 = MJD[numarray.argmax(phi_2)]
    t_02 = MJD[numarray.argmax(sat_r)]
    t_00 = MJD[numarray.argmax(sat_theta)]
    last_i = i
    last_i_0 = i_0
    last_t_00 = t_00
    last_chi_2 = 1E300
    ...
    for j in xrange(iterations):
        ...
        if(chi_2 > last_chi_2):
            ...
            if(factor > 0.5):
                factor = 0.5
        slope = 2.0 * math.pi * i / T_day * numarray.sin(inside)
    ...
    for j in xrange(iterations):
        ...
        diff = diff - beta * numarray.cos(inside)
        diff2 = diff*diff
        chi_2 = diff2.sum()
        if(chi_2 > last_chi_2):
            t_01 = last_t_01
            ...
            if(factor > 0.5):
                factor = 0.5
        slope = 4.0 * math.pi * beta / T_day * numarray.sin(inside)
        slope = -2.0 * diff * slope
        d_sum = slope.sum()
        if(d_sum == 0.0):
            break
    ...
    for j in xrange(iterations):
        ...
        if(chi_2 > last_chi_2):
            ...
            if(factor > 0.5):
                factor = 0.5
        slope = 2.0 * math.pi * r_A / T_day * numarray.sin(inside)
        slope = -2.0 * diff * slope
        d_sum = slope.sum()
        if(d_sum == 0.0):
            break
...
"""
    try:
        ...
        MJD_start = jma_aips.get_MJD_hms(year, month, day, hour, minute, second)
        Num_Data = int(line[32:39])
        if(Num_Data < 10):
            raise RuntimeError("Too few data times in file '%s' to get accurate interpolation"%filename)
        # skip to line 13
        ...
        for i in xrange(14,23):
            line = fp.readline()
        # Now to the data
        MJD = numarray.zeros((Num_Data), type='Float64')
        obs_data = numarray.zeros((Num_Data, MAX_POSSIBLE_SATELLITES, 6),
                                  type='Float64')
        time_count = -1
        while(1):
            line = fp.readline()
            ...
            if(line[0] == '*'):
                ...
                MJD[time_count] = jma_aips.get_MJD_hms(year, month, day,
                                                       hour, minute, second)
            elif(line[0] == 'P'):
                code = line[1]
                sat = int(line[2:4])
                if((code == ' ') or (code == 'G')):
                    ...
                    raise RuntimeError("Unsupported satellite type '%s' in SP3 at line '%s'"%(code,line))
                assert(sat >= 0)
                assert(sat < MAX_POSSIBLE_SATELLITES)
                ...
                try:
                    ...
                    if(r == 0.0):
                        ...
                        raise RuntimeError("Unknown failure for %E %E %E"%(x,y,z))
                ...
                raise RuntimeError("Unsupported data type in line '%s'"%line)
    finally:
        fp.close()
...
MJD         I  numarray of times, as MJD[]
sat_pos     I  numarray of satellite positin information, as
...
work_c      I  A numarray work space, of double type.  Must be
               of the form work_c[num_terms+], where the
               plus signes indicate minimum dimension size
work_d      I  A numarray work space, of double type.  Must be
               of the form work_d[num_terms+], where the
               plus signes indicate minimum dimension size

OUTPUTS:
XYZ         O  numarray of the satellite position
               x=0,y=1,z=2
"""
    assert(work_c.getshape()[0] >= num_terms)
    assert(work_d.getshape()[0] >= num_terms)
    XYZ = numarray.zeros((6), type='Float64')
    for p in xrange(3,6):
        ...
        ns -= 1
        try:
            for m in xrange(1,num_terms):
                for i in xrange(num_terms-m):
...
sat_pos     I  numarray of satellite positin information, as
               ...
               This is expected to come from the obs_data output from
               read_RINEX_sp3_file.
MJD_need    I  The desired MJDs of the satellite positions, as a numarray
               MJD_need[need_times]

OUTPUTS:
XYZ         O  numarray of the satellite positions
...
"""
    ...
    assert(NUM_OBS == MJD.getshape()[0])
    NUM_NEED = MJD_need.getshape()[0]
    assert(NUM_NEED>0)
    XYZ = numarray.zeros((NUM_NEED,NUM_SAT,6), type='Float64')
    work_c = numarray.zeros((NUM_TERMS), type='Float64')
    work_d = numarray.zeros((NUM_TERMS), type='Float64')
    for s in xrange(NUM_SAT):
        ...
        pos_res = numarray.reshape(pos_res, (NUM_OBS,1,NUM_DIR))
        inside = 2.0*math.pi * (MJD-orbital_param[9])/orbital_param[2]
        pos_res[:,0,3] += - orbital_param[5] - orbital_param[6] * numarray.cos(inside)
        inside = 4.0*math.pi * (MJD-orbital_param[8])/orbital_param[2]
        pos_res[:,0,4] += - orbital_param[3] - 2.0 * math.pi * (1.0/orbital_param[2] - 1.0) * MJD - orbital_param[4] * numarray.cos(inside)
        inside = 2.0*math.pi * (MJD-orbital_param[7])/orbital_param[2]
        pos_res[:,0,5] += - 0.5*math.pi - orbital_param[0] * numarray.cos(inside) - orbital_param[1]
        # Get the predicted orbit values
        inside = 2.0*math.pi * (MJD_need-orbital_param[9])/orbital_param[2]
        XYZ[:,s,3] = orbital_param[5] + orbital_param[6] * numarray.cos(inside)
        inside = 4.0*math.pi * (MJD_need-orbital_param[8])/orbital_param[2]
        XYZ[:,s,4] = orbital_param[3] + 2.0 * math.pi * (1.0/orbital_param[2] - 1.0) * MJD_need + orbital_param[4] * numarray.cos(inside)
        inside = 2.0*math.pi * (MJD_need-orbital_param[7])/orbital_param[2]
        XYZ[:,s,5] = 0.5*math.pi + orbital_param[0] * numarray.cos(inside) + orbital_param[1]
...
"""
    ...
    assert(NUM_NEED>0)
    XYZ = numarray.zeros((NUM_NEED,NUM_SAT,6), type='Float64')
    work_c = numarray.zeros((NUM_TERMS_MAX), type='Float64')
    work_d = numarray.zeros((NUM_TERMS_MAX), type='Float64')
    for s in xrange(NUM_SAT):
        if((sat_pos[0,s,0] == 0.0)and(sat_pos[0,s,0] == 0.0)
           and(sat_pos[0,s,0] == 0.0)):
            continue
...
sat_XYZ    I  satellite position in Earth centered coodriantes, in m
              Must be a numarray with 6 elements(x,y,z,r,phi,theta)
...
"""
    ...
    try:
        ZA = math.acos(dot_p/(r_sta*r_sat))
    except ArithmeticError:
        if(dot_p > 0.0):
            ZA = 0.0
        elif(dot_p < 0.0):
...
MJD        I  numarray of Modified Julian Dates for each observation time
              as MJD[num_times]
Sat_array  I  numarray of satellite data positions, as
              Sat_array[num_times,MAX_POSSIBLE_SATELLITES]
              If the value is -1, then no observation of that satellite was made
...
sta_XYZ    I  Cartesian station position in Earth centered coodriantes, in m
              Must be a numarray
sat_XYZ    I  numarray of the satellite positions
              as XYZ[num_times,MAX_POSSIBLE_SATELLITES,3]
              x=0,y=1,z=2.  If x==y==z==0, the no data

OUTPUTS: None
"""
    NUM_OBS = obs_data.getshape()[0]
    assert(NUM_OBS == MJD.getshape()[0])
    assert(NUM_OBS == Sat_array.getshape()[0])
    assert(NUM_OBS == sat_XYZ.getshape()[0])
    El_pos = _DATA_POS['EL']
    Az_pos = _DATA_POS['AZ']
    for i in xrange(NUM_OBS):
        for s in xrange(MAX_POSSIBLE_SATELLITES):
...
"""
    ...
    SNR_FLAG = 0
    for i in xrange(obs_data.getshape()[0]):
        for sat in xrange(obs_data.getshape()[1]):
        ...
        if(SNR_FLAG == 1): break
    for i in xrange(obs_data.getshape()[0]):
        for sat in xrange(MAX_POSSIBLE_SATELLITES):
            s = Sat_array[i,sat]
            if(s < 0):
                continue
            ...
            if((P1 != BAD_DATA_CODE)and(P2 != BAD_DATA_CODE)):
                ...
                if(math.fabs(STEC) < MAX_PSEUDORANGE_DIFFERENCE):
                    ...
                    if((sat >= 0)and(sat < 100)):
                        ...
                        raise RuntimeError("Unknown satellite type %d"%sat)
...
INPUTS:
MJD                        I  numarray of Modified Julian Dates for each
...
"""
    ...
    for sat in xrange(MAX_POSSIBLE_SATELLITES):
        ...
        if(sat < 100):
            ...
            raise RuntimeError, "Unknown satellite type for satellite %d"%sat
        ...
        for i in xrange(NEXT_START, NUM_OBS):
            s = Sat_array[i,sat]
            if(s >= 0):
                if(obs_data[i,s, P2_pos] != BAD_DATA_CODE):
                    obs_data[i,s, P2_pos] += SPEED_OF_LIGHT \
    ...
    sta_bias_valid = numarray.zeros((MAX_POSSIBLE_SATELLITES/100),type='Int32')+1
...
OUTPUTS: None
    ...
    if(adiff > 0.2) :
        ...
        for xx1 in xrange(-2,3):
            for xx2 in xrange(-2,3):
...
work       W  a double numarray of size work[NUM_OBS] for workspace
...
"""
    if(sat >= MAX_POSSIBLE_SATELLITES):
        raise RuntimeError("Unsupported satellite number %d, contact software provider"%(sat))
    TP_pos = _DATA_POS['STECP']
    TL_pos = _DATA_POS['STECL']
    if(sat < 100):
        ...
        if(last_L == BAD_DATA_CODE):
            ...
            if(start_pos - end_pos <= MAX_SPIP_POINTS):
                if(end_pos != 0):
                    ...
                    for j in xrange(end_pos,start_pos):
    ...
    for pos in block_pos:
        ...
        while(this_start < end_pos):
            ...
            if(this_end != end_pos):
                ...
                if(slip != 0.0):
                    for j in xrange(next_start,end_pos):
                        s = Sat_array[j,sat]
                        if(s < 0):
                            continue
                        L = obs_data[j,s, TL_pos]
...
"""
    ...
    if(numarray.sum(Sat_array[:,sat]) == -NUM_OBS):
        return 0, []
    TP_pos = _DATA_POS['STECP']
    TL_pos = _DATA_POS['STECL']
    if(sat < 100):
    ...
    if(std_dev > 0.0):
        ...
        return sat_found, block_pos
    if(num > 10):
        diff_sort = numarray.sort(work.copy())
        start_d = -1
        end_d = len(diff_sort)
        for index in xrange(end_d):
...
"""
    ...
    NUM_OBS = obs_data.getshape()[0]
    work = numarray.zeros((NUM_OBS), type='Float64')
        ...
        if(code == 1):
            ...
            for i in xrange(NUM_OBS):
                ...
                if(obs_data[i,s, SIG_pos] != BAD_DATA_CODE):
                    ...
                    count += 1
            average = numarray.zeros((1), type='Float64')
            if(count):
                average[0] = sum/count
                total_sat_count += 1
            else:
            ...
            continue
        average = numarray.zeros((len(block_pos)), type='Float64')
        average += BAD_DATA_CODE
        for pos in xrange(len(block_pos)):
    ...
    average_arr = numarray.zeros((total_sat_count), type='Float64')
    count = 0
    for sat in xrange(MAX_POSSIBLE_SATELLITES):
        if((sat_avg_arr[sat] is not None) and (sat_block_pos[sat] is not None)):
            for pos in xrange(len(sat_avg_arr[sat])):
    ...
    average_use = numarray.sort(average_use)
    if(count >= 20):
            ...
            for pos in xrange(len(sat_avg_arr[sat])):
                if((sat_avg_arr[sat][pos] == BAD_DATA_CODE) or
                   (math.fabs(sat_avg_arr[sat][pos] - mean) > check)):
                    ...
                    for i in xrange(sat_block_pos[sat][pos][0],
...
def concatenate_MJDs(MJDs):
    """Concatenate a set of modified Julian Date numarrays into a single one.
...
INPUTS:
MJDs      I  list[] of the MJD numarrays you want to concat.

OUTPUTS: MJD_out
MJD_OUT   O  the whole numarray of MJD values
"""
    assert(len(MJDs) > 0)
    NUM_TOTAL = 0
    for M in MJDs:
        NUM_TOTAL += len(M)
    MJD_OUT = numarray.zeros((NUM_TOTAL), type='Float64')
    last_val = MJDs[0][0]
    gap = 0
    hour = 1.0 / 24.0
    reverse = 0
...
def concatenate_Sat_Arrs(Sat_Arrs):
    """Concatenate a set of satellite observation numarrays into a single one.
...
OUTPUTS: MJD_out
Sat_Arr_OUT   O  the whole numarray of satellite stuff
                 numarray of satellite data positions, as
                 Sat_array[num_times,MAX_POSSIBLE_SATELLITES]
...
"""
    ...
    Sat_Arr_OUT = numarray.zeros((NUM_TOTAL, MAX_POSSIBLE_SATELLITES),
                                 type='Int16')
    pos = 0
    for S in Sat_Arrs:
        l = S.getshape()[0]
...
def concatenate_Obs_Datas(Obs_Datas):
    """Concatenate a set of GPS observation numarrays into a single one.
...
OUTPUTS: MJD_out
Obs_Data_OUT O  the whole numarray of observation stuff
...
"""
    ...
    Obs_Data_OUT = numarray.zeros((NUM_TOTAL, NUM_MAX, _DATA_POS_SIZE),
                                  type='Float64')
...
OUTPUTS: MJD_out
Sat_Obss_OUT O  the whole numarray of satellite observation stuff
              The output data array, as
              obs_data[num_times, MAX_POSSIBLE_SATELLITES, 6]
              Assuming you have found the best index i in MJD for
              a time you desire, and you want data for satellite s, then
...
"""
    ...
    for S in Sat_Obss:
        ...
        assert(sh[1] == MAX_POSSIBLE_SATELLITES)
        assert(sh[2] == 6)
    Sat_Obss_OUT = numarray.zeros((NUM_TOTAL, MAX_POSSIBLE_SATELLITES, 6),
                                  type='Float64')
    pos = 0
    for S in Sat_Obss:
        l = S.getshape()[0]
...
INPUTS:
sta_XYZs     I  list[] of station positions as numarrays of X,Y,Z coordinates
...
"""
    ...
    assert(NUM_TOTAL > 0)
    max_dist = 0.0
    for i in xrange(NUM_TOTAL):
        p_i = sta_XYZs[i]
        for j in xrange(i+1,NUM_TOTAL):
            d = p_i - sta_XYZs[j]
...
"""
    assert((len(MJD_1.getshape())==1) and (len(MJD_2.getshape()) ==1))
    NUM_DATA = len(MJD_1)
    ...
    for i in xrange(NUM_DATA):
        diff = math.fabs(MJD_1[i] - MJD_2[i])
        if(diff > epsilon):
            #print i, MJD_1[i], MJD_2[i]
            return +1
    return 0
...
OUTPUTS: Sat_small
Sat_small    O  numarray of satellite data positions, as
...
"""
    osh = obs_data.getshape()
    ssh = Sat_array.getshape()
    assert(osh[0] == ssh[0])
    Sat_small = numarray.zeros((osh[0],osh[1]), type='Int16')
    Sat_small -= 1
    for t in xrange(osh[0]):
        for s in xrange(ssh[1]):
            col = Sat_array[t,s]
            if(col >= 0):
                Sat_small[t,col] = s
    return Sat_small
...
OUTPUTS: Sat_small
block_small  O  numarray of satellite track positions, as
...
"""
    ...
    block_small = numarray.zeros((osh[0],osh[1]), type='Int16')
    block_small -= 1
    for s in xrange(ssh[1]):
        if(sat_block_pos[s] is None):
            continue
        for p in xrange(len(sat_block_pos[s])):
            for t in xrange(sat_block_pos[s][p][0],sat_block_pos[s][p][1]):
                col = Sat_array[t,s]
                if(col >= 0):
                    block_small[t,col] = p
    return block_small
...
sat_MJD     O  numarray of Modified Julian Dates for each ephemeris time
               as MJD[num_times]
...
"""
    ...
    sat_MJD_list = []
    sat_pos_list = []
    for m in xrange(MJD_start, MJD_end):
        gps_week, gps_dow, gps_seconds = Albus_Iono.get_GPS_from_MJD(m)
        filename = Albus_RINEX.make_RINEX_ephemeris_filename("cod", gps_week,
                                                             gps_dow)
        ret_code = Albus_RINEX.get_GPS_ephemeris_file_from_web(filename,
                                                               gps_week,
                                                               output_directory,
                                                               overwrite)
        if(ret_code < 0):
            raise RuntimeError("Could not download ephemeris data for MJD %d"%m)
...
"""
    ...
    year, month, day, frac = jma_aips.get_ymdf_from_JD(jma_aips.get_JD_from_MJD(MJD))
    doy = jma_aips.get_day_of_year(year, month, day)
...
sta_MJD      O  numarray of Modified Julian Dates for each observation time
                as MJD[num_times]
Sat_array    O  numarray of satellite data positions, as
                Sat_array[num_times,MAX_POSSIBLE_SATELLITES]
                If the value is -1, then no observation of that satellite was
...
"""
    ...
    MJD_start = int(MJD_start) # int truncates!
    MJD_end = int(MJD_end) + 1
    for m in xrange(MJD_start-1, MJD_end+1):
        year, month, day, frac = jma_aips.get_ymdf_from_JD(jma_aips.get_JD_from_MJD(m))
        doy = jma_aips.get_day_of_year(year, month, day)
    ...
    if(calc_station_position_max_shift(sta_XYZ_list) < 100.0):
        ...
        raise RuntimeError("Hey, the station is moving, maximum station difference is %.3f meters"%calc_station_position_max_shift(sta_XYZ_list))
...
OUTPUTS: MJD
MJD          O  A numarray of MJD times.  If no valid data, this comes back as
                None
    """
    MJD = None
    ...
    if(not os.path.isfile(filename)):
        return MJD
    m_fp = numarray.memmap.open(filename, "r", len=4)
    try:
        slice = m_fp[0:4]
        Num_El = numarray.NumArray(buffer=slice,
                                   shape=(1),
                                   type='UInt32')
        Num_El = Num_El.copy()[0]
        MJD_SIZE = Num_El*8
        m_fp.close()
        m_fp = numarray.memmap.open(filename, "r", len=4+MJD_SIZE)
        slice2 = m_fp[4:4+MJD_SIZE]
        MJD = numarray.NumArray(buffer=slice2,
                                shape=(Num_El),
                                type='Float64')
        MJD = MJD.copy()
    finally:
...
INPUTS:
MJD          I  A numarray of MJD times.  If no valid data, this comes back as
                None
MJD_start    I  The MJD of the starting time needed
MJD_end      I  The MJD of the ending time needed
station_code I  String of letters indicating the origin of the times
...
OUTPUTS: None
    ...
    sh = MJD.getshape()
    MJD_SIZE = sh[0]*8
    m_fp = numarray.memmap.open(filename, "w+", len=4+MJD_SIZE)
    try:
        ...
        # write out the data
        slice2 = m_fp[4:4+MJD_SIZE]
        MJD_out = numarray.NumArray(buffer=slice2,
                                    shape=(sh[0]),
                                    type='Float64')
        MJD_out[:] = MJD[:]
    finally:
...
sat_MJD      O  A numarray of MJD times corresponding to the satellite positions
                If no valid data, this comes back as None
sat_XYZ      O  numarray of the satellite positions
    ...
    if(not os.path.isfile(filename)):
        return None, None
    m_fp = numarray.memmap.open(filename, "r", len=3*4)
    try:
        # read the header
        slice = m_fp[0:3*4]
        Num_El = numarray.NumArray(buffer=slice,
        ...
        m_fp.close()
        m_fp = numarray.memmap.open(filename, "r", len=3*4+XYZ_SIZE)
        # read the data
        slice2 = m_fp[3*4:3*4+XYZ_SIZE]
        sat_XYZ = numarray.NumArray(buffer=slice2,
                                    shape=(Num_El[0],Num_El[1],Num_El[2]),
                                    type='Float64')
        sat_XYZ = sat_XYZ.copy()
    finally:
        m_fp.close()
...
INPUTS:
sat_MJD      I  A numarray of MJD times corresponding to the satellite positions
                If no valid data, this comes back as None
sat_XYZ      I  numarray of the satellite positions
                as XYZ[need_times,MAX_POSSIBLE_SATELLITES,6]
                x=0,y=1,z=2.  If x==y==z==0, then no data
...
OUTPUTS: None
    ...
    m_fp = numarray.memmap.open(filename, "w+", len=3*4+XYZ_SIZE)
    try:
        # write out the header
        slice = m_fp[0:3*4]
        Num_El = numarray.NumArray(buffer=slice,
        ...
        # write out the data
        slice2 = m_fp[3*4:3*4+XYZ_SIZE]
        sat_out = numarray.NumArray(buffer=slice2,
...
Sat_array    O  numarray of satellite data positions, as
    ...
    if(not os.path.isfile(filename)):
        return None
    m_fp = numarray.memmap.open(filename, "r", len=2*4)
    try:
        # read the header
        slice = m_fp[0:2*4]
        Num_El = numarray.NumArray(buffer=slice,
        ...
        Num_El = Num_El.copy()
        assert(Num_El[1] == MAX_POSSIBLE_SATELLITES)
        ARRAY_SIZE = Num_El[0]*MAX_POSSIBLE_SATELLITES*2
        m_fp.close()
        m_fp = numarray.memmap.open(filename, "r", len=2*4+ARRAY_SIZE)
        # read the data
        slice2 = m_fp[2*4:2*4+ARRAY_SIZE]
        Sat_array = numarray.NumArray(buffer=slice2,
                                      shape=(Num_El[0],Num_El[1]),
                                      type='Int16')
        Sat_array = Sat_array.copy()
    finally:
        m_fp.close()
    return Sat_array
...
OUTPUTS: None
    ...
    if(os.path.isfile(filename)):
        if(overwrite):
            ...
            return
    ARRAY_SIZE = sh[0]*MAX_POSSIBLE_SATELLITES*2
    m_fp = numarray.memmap.open(filename, "w+", len=2*4+ARRAY_SIZE)
    try:
        # write out the header
        slice = m_fp[0:2*4]
        Num_El = numarray.NumArray(buffer=slice,
...
obs_data     O  The output data array, as
    ...
    try:
        ...
        OBS_SIZE = Num_El[0]*Num_El[1]*_DATA_POS_SIZE*8
        m_fp.close()
        m_fp = numarray.memmap.open(filename, "r", len=3*4+OBS_SIZE)
        # read the data
        slice2 = m_fp[3*4:3*4+OBS_SIZE]
        obs_data = numarray.NumArray(buffer=slice2,
                                     shape=(Num_El[0],Num_El[1], _DATA_POS_SIZE),
                                     type='Float64')
...
OUTPUTS: None
    ...
    if(os.path.isfile(filename)):
        if(overwrite):
            ...
            return
    OBS_SIZE = sh[0]*sh[1]*_DATA_POS_SIZE*8
    m_fp = numarray.memmap.open(filename, "w+", len=3*4+OBS_SIZE)
    try:
        # write out the header
        slice = m_fp[0:3*4]
        Num_El = numarray.NumArray(buffer=slice,
        ...
        # write out the data
        slice2 = m_fp[3*4:3*4+OBS_SIZE]
        obs_out = numarray.NumArray(buffer=slice2,
                                    shape=(sh[0],sh[1],_DATA_POS_SIZE),
                                    type='Float64')
        obs_out[:,:,:] = obs_data[:,:,:]
    finally:
        m_fp.close()
...
sta_bias_valid O  An array of flags to indicate whether station bias
    ...
    m_fp = numarray.memmap.open(filename, "r", len=6*8+BIAS_LENGTH*4)
    try:
        # read the position
        slice = m_fp[0:6*8]
        sta_XYZ = numarray.NumArray(buffer=slice,
                                   shape=(6),
                                   type='Float64')
        sta_XYZ = sta_XYZ.copy()
        # read the bias descriptors
        slice2 = m_fp[6*8:6*8+BIAS_LENGTH*4]
        sta_bias_valid = numarray.NumArray(buffer=slice,
                                   shape=(BIAS_LENGTH),
                                   type='Int32')
        sta_bias_valid = sta_bias_valid.copy()
    finally:
        m_fp.close()
...
OUTPUTS: None
    ...
    if(os.path.isfile(filename)):
        if(overwrite):
            ...
            return
    m_fp = numarray.memmap.open(filename, "w+", len=6*8+BIAS_LENGTH*4)
    try:
        # write out the position
        slice = m_fp[0:6*8]
        XYZ_out = numarray.NumArray(buffer=slice,
                                   shape=(6),
                                   type='Float64')
        XYZ_out[:] = sta_XYZ[:]
        # write out the bias descriptors
        slice2 = m_fp[6*8:6*8+BIAS_LENGTH*4]
        bias_out = numarray.NumArray(buffer=slice,
                                     shape=(BIAS_LENGTH),
                                     type='Int32')
...
sat_block_pos  O  An array of continguous block position ranges, as 3 element
    ...
    try:
        for s in xrange(MAX_POSSIBLE_SATELLITES):
            line = fp.readline()
            assert(line[0] == 'S')
            array = map(int,line[1:-1].strip().split(None,2)[0:2])
            assert(array[0] == s)
            sat_block_pos[s] = [None]*array[1]
            for p in xrange(array[1]):
                line = fp.readline()
                assert(line[0] == 'T')
                track = map(int,line[1:-1].strip().split(None,4)[0:4])
...
sta_MJD      I  The array of Modified Julian Dates for each observation
                time as MJD[num_times], as a numarray.  This should be from
...
sat_XYZ      O  numarray of the satellite positions
    ...
    if(use_bin_files):
        ...
        if((sat_MJD is not None) and (sat_XYZ is not None)):
            retval = check_MJD_arrays_differ(sat_MJD, sta_MJD)
            if(retval == 0):
                return sat_XYZ
...
sta_MJD_in   I  The previous array of Modified Julian Dates for each observation
                time as MJD[num_times], as a numarray.  May also be None
sat_XYZ_in   I  The previous numarray of the satellite positions
                as XYZ[need_times,MAX_POSSIBLE_SATELLITES,6]
                x=0,y=1,z=2.  If x==y==z==0, then no data
                r=3,\phi=4,\theta=5
...
"""
    ...
    if(sta_MJD_in is not None):
        retval = check_MJD_arrays_differ(sta_MJD, sta_MJD_in, 0.5)
        if(retval):
            raise RuntimeError("Error: MJD dates differ (%d)"%(retval))
    else:
        sat_XYZ_in = None
    if(sat_XYZ_in == None):    
        sat_XYZ = get_satellite_full_positions(MJD_start, MJD_end, sta_MJD,
...
M_s         O  The MeqTree satellite number.  Numbers >= 0 are valid.
    ...
    """
    for type in range(MAX_POSSIBLE_SATELLITES/100):
        offset = sat_number - type*100
...
R_s         O  The RINEX satellite number.  Numbers >= 0 are valid.
    ...
    if(sat_number < 0):
        return -1
    type = int(sat_number/50)
    offset = sat_number - type*50
    R_s = offset + type*100
...
OUTPUTS: None
    ...
    try:
        m_fp = None
        m_fp = numarray.memmap.open(filename, "w+", len=SATELLITE_SIZE)
        slice = m_fp[0:SATELLITE_SIZE]
        sat_out = numarray.NumArray(buffer=slice,
                                    shape=(NUM_TIMES,
                                           MAX_POSSIBLE_MEQTREE_SATELLITES,
                                           3),
                                    type='Float64')
        sat_out[:,:,:] = 0.0  # place blank satellites at the center of the Earth
        for s in xrange(MAX_POSSIBLE_MEQTREE_SATELLITES):
...
STEC         I  numarray of STEC values in m^{-2}, of the form
                STEC[num_times, max_sat]
sigma        I  numarray of STEC uncertainty values in m^{-2}, of the form
...
OUTPUTS: None
    ...
    try:
        m_fp = None
        m_fp = numarray.memmap.open(filename, "w+", len=STATION_TOTAL_SIZE)
        slice = m_fp[0:STATION_STEC_SIZE]
        sta_out = numarray.NumArray(buffer=slice,
                                    shape=(NUM_TIMES,
                                           MAX_POSSIBLE_MEQTREE_SATELLITES,
                                           2),
                                    type='Float32')
        ...
        sta_pos = numarray.NumArray(buffer=slice2,
                                    shape=(3),
                                    type='Float64')
        sta_pos[:] = sta_XYZ[0:3]
    finally:
...
OUTPUTS: None
    ...
    fp = open(filename, "w")
    try:
        for ms in xrange(MAX_POSSIBLE_MEQTREE_SATELLITES):
...
OUTPUTS: None
    ...
    MJD_array = None
    sat_XYZ   = None
    for i in xrange(len(station_code_list)):
