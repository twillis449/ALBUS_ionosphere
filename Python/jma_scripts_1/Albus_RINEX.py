...
################################################################################
# miscellaneous stuff
import copy, optparse, os, sys, signal, popen2, getpass
import re, string
import numarray
import numarray.ma
...
_RINEX_SITE_LIST_Aus_0 = ["ckis","kiri","laut","naur","pngm","pohn","samo","tong","tuva","vanu"]
_RINEX_SITE_LIST_Aus_1_all = ["alic","bur1","cas1","cedu","coco","darm","darr","darw","dav1","hil1","hob2","jab1","karr","mac1","maw1","mobs","nml1","park","str1","str2","sydn","tid1","tow2","xmis","yar2","yarr"]
_RINEX_SITE_LIST_Aus_1 = ["bur1","darm","hil1","nml1","park","xmis"]
_RINEX_SITE_LIST_Ned_0 = ["apel","cabw","delf","dlft","eijs","kosg","ters","wsra","wsrt"]
...
"""
    ...
    #urllib.urlretrieve(site_str, our_Z_file, urlreporthook)
    #urllib.urlcleanup()
    try:
...
get_url.computer_dict["gnss1.lr.tudelft.nl"] = [None,None]
...
################################################################################
def get_RINEX_obs_file_from_web(RINEX_filename,
                                year, month, day, doy,
                                output_directory = ".",
                                FTP_site = 0,
                                overwrite = 0
                                ):
    """use urllib to get a RINEX station observation file from the Internet
...
"""
    ...
    if(FTP_site > 16):
        ...
        if(RINEX_filename[-1] != 'd'):
            ...
            if(os.path.isfile(new_file)):
                ...
                try:
                    get_RINEX_obs_file_from_web(RINEX_filename[:-1]+'o',
                                                year, month, day, doy,
                                                output_directory,
                                                0,
                                                overwrite)
    ...
    if(FTP_site == 0):
        if(RINEX_filename[0:4] in _RINEX_SITE_LIST_Aus_0):
            FTP_site = 13
        elif(RINEX_filename[0:4] in _RINEX_SITE_LIST_Aus_1):
            FTP_site = 13
        elif(RINEX_filename[0:4] in _RINEX_SITE_LIST_Ned_0):
            FTP_site = 14
        elif(RINEX_filename[0:4] in _RINEX_SITE_LIST_BIGF_0):
            FTP_site = 15
        elif(RINEX_filename[0:4] in _RINEX_SITE_LIST_AOPR_0):
            FTP_site = 16
    assert(year > 1979)
    assert(year < 2080)
    yy = year - 1900
    ...
    if(FTP_site == 0):
        ...
        # EUREF in France, plus a few French stations
        site_str = "ftp://lareg.ensg.ign.fr/pub/rgp/%4.4d/%3.3d/%s.Z"%(year, doy, RINEX_filename)
    elif(FTP_site == 9):
        ...
        # Australia site
        if(RINEX_filename[0:4] in _RINEX_SITE_LIST_Aus_0):
            site_str = "ftp://ftp.ga.gov.au/geodesy-outgoing/sgac/spslcmp/gpsdata/%2.2d%3.3d/%s.Z"%(yy, doy, RINEX_filename)
        elif(RINEX_filename[0:4] in _RINEX_SITE_LIST_Aus_1_all):
            site_str = "ftp://ftp.ga.gov.au/geodesy-outgoing/gpsdata/%2.2d%3.3d/%s.Z"%(yy, doy, RINEX_filename)
        else:
        ...
        if(RINEX_filename[0:4] in _RINEX_SITE_LIST_Ned_0):
            site_str = "ftp://gnss1.lr.tudelft.nl/d1/rinex/%4.4d/%3.3d/%s.Z"%(year, doy, RINEX_filename)
        else:
            pass # site_str = "" from above, which will generate an IOError
    elif(FTP_site == 15):
...
def get_GPS_ephemeris_file_from_web(ephemeris_filename,
    ...
    """use urllib to get an ephemeris file from the Internet
...
def get_IONEX_file_from_web(IONEX_filename,
    ...
    """use urllib to get an IONEX file from the Internet
...
"""
    ...
    if(FTP_site == 0):
        # IGN France
        site_str = "ftp://igs.ensg.ign.fr/pub/igs/iono/%4.4d/%3.3d/%s.Z"%(year, doy, IONEX_filename)
    elif(FTP_site == 1):
        # CDDIS Goddard
...
def get_CODE_P1P2_file_from_web(year, month, data_type = 'P2',
    ...
    """use urllib to get a P1P2 differential code bias file from CODE
...
"""
    ...
    site_str = "ftp://ftp.unibe.ch/aiub/CODE/%4.4d/%s.Z"%(year, filename)
    try:
        if(data_type == 'P2'):
            get_url(site_str, our_Z_file)
        else:
...
"""
    ...
    try:
        ...
        raise No_RINEX_File_Error("Error: IONEX file '%s' cannot be opened"%filename)
    for row in fp:
...
def get_RINEX_station_XYZ(rinex_file, station_code = None):
        ...
        try:
            for row in fp:
                if(row[60:79] == "APPROX POSITION XYZ"):
                    xyz = map(float,row.strip().split(None,3)[0:3])
                    fp.close()
                    return xyz
        finally:
            fp.close()
    ...
    if(station_code in GPS_stations.GPS_stations):
        ...
        raise RuntimeError, "Error: could not find station position in '%s' or the GPS station list"%rinex_file
...
MJD      O  numarray(i) of the MJDs of the observations
Az       0  numarray(i,s) of the Azimuths for sat s, in radians
El       0  numarray(i,s) of the Azimuths for sat s, in radians
            If -999.0, then no data present for any of Az, El, STEC
STEC     0  numarray(i,s) of the STEC values for sat s, in TECU
            If -999.0, then no data present for any of Az, El, STEC
"""
    ...
    for row in fp:
        ...
        if(r[2][0] == 'G'):
            ...
            fp.close()
            raise RuntimeError, "Unknown satellite type"
        if(sat > sat_max): sat_max = sat
        if(sat < sat_min): sat_min = sat
        MJD_here = Albus_Iono.get_MJD_from_GPS_week(int(r[0]),
                                                    float(r[1]))
    ...
    MJD = numarray.zeros((num_rows), type='Float64')
    Az = numarray.zeros((num_rows,sat_max), type='Float64')
    El = numarray.zeros((num_rows,sat_max), type='Float64')
    STEC = numarray.zeros((num_rows,sat_max), type='Float64')
...
MJD      I  numarray(i) of the MJDs of the observations
Az       I  numarray(i,s) of the Azimuths for sat s, in radians
El       I  numarray(i,s) of the Azimuths for sat s, in radians
            If -999.0, then no data present for any of Az, El, STEC
STEC     I  numarray(i,s) of the STEC values for sat s, in TECU
            If -999.0, then no data present for any of Az, El, STEC
filename I  name of path+file to write to
overwrite I may this function overwrite files? 0 No, else yes
"""
    assert(STEC.getshape() == El.getshape())
    assert(STEC.getshape() == Az.getshape())
    assert(STEC.getshape()[0] == MJD.getshape()[0])
    if(os.path.isfile(filename)):
        if(overwrite):
            warnings.warn("File '%s' already exists.  Deleting."%filename)
            os.remove(filename)
        else:
            raise RuntimeError, "Error: file '%s' already exists and not allowed to delete"%filename
    fp = open(filename, "w")
    for n in xrange(STEC.getshape()[0]):
        for s in xrange(STEC.getshape()[1]):
...
"""
    ...
    for f in filenames:
        ...
        for row in fp:
            ...
            if(MJD_here < MJD_last):
                fp.close()
                raise RuntimeError, "Error, time not increasing"
            if(MJD_here != MJD_last):
                MJD_last = MJD_here
                num_rows += 1
        fp.close()
...
INPUTS:
STEC                       I  A numarray array for slant TEC values, as
...
"""
    ...
    for sat in xrange(STEC.getshape()[1]):
        # satellite
...
Take in a list of STEC numarray arrays.  For each satellite, over all stations,
...
S            I  list of STEC numarray arrays
E            I  corresponding list of Elevation numarray arrays
...
"""
    ...
    max_num_stations = 0
    for stec in S:
        num_sat = stec.getshape()[1]
        if(num_sat > max_num_stations):
            max_num_stations = num_sat
    for s in xrange(max_num_stations):
        min_STEC = +1E300
        min_STEC2 = +1E300
        for stec, el in zip(S,E):
            if(stec.getshape()[1] <= s): continue
            for n in xrange(stec.getshape()[0]):
...
"""
    ...
    MJD_start = jma_aips.get_MJD(year, month, day)
    ...
    data_file_list = []
    for MJD in xrange(int(MJD_start),int(MJD_end)+1):
        print "Doing day", MJD
        # create the bias corrected filename
        y,m,d,f = jma_aips.get_ymdf_from_JD(jma_aips.get_JD_from_MJD(MJD))
        doy = jma_aips.get_day_of_year(y,m,d)
        ...
        if((os.path.isfile(RINEX_B_path)) and (not main_file_overwrite)):
            ...
            if((os.path.isfile(RINEX_D_path)) and (not overwrite)):
                ...
                data_in = make_RINEX_filename(station_code,'d', y, doy)
                nav_in = make_RINEX_filename('brdc','n', y, doy)
                nav_gl = make_RINEX_filename('brdc','g', y, doy)
                try:
                    get_RINEX_obs_file_from_web(data_in, y,m,d,doy,
                                                data_directory,
                                                overwrite = overwrite)
                    ...
                    try:
                        get_RINEX_obs_file_from_web(nav_gl, y,m,d,doy,
                                                    data_directory,
                                                    overwrite = overwrite)
                    except No_RINEX_File_Error:
                        pass
...
Given some generic (probably numarray) array (1-D), which is in increasing
...
Given an array (probably a numarray) of MJD times in MJD, and a window
...
is pythonish, so that you can use xrange(index_bottom, index_top).
...
index_bottom   O  the bottom index to use for an xrange(index_bottom, index_top)
                  call.  If there are no MJDs in the window, then this
                  is None
index_top      O  the top index to use for an xrange(index_bottom, index_top)
                  call.  If there are no MJDs in the window, then this
                  is None
...
last_index     I  The last index as to where the nearest time point was
                  found.  Set to None to start over
MJD            I  numarray(i) of the MJDs of the RINEX data
Az             I  numarray(i,s) of the Azimuths for sat s, in radians
El             I  numarray(i,s) of the Azimuths for sat s, in radians
                  If -999.0, then no data present for any of Az, El, STEC
STEC           I  numarray(i,s) of the STEC values for sat s, in TECU
...
"""
    ...
    # wants seconds
    MJD_base = jma_aips.get_MJD(year, month, day)
    ...
    if(retval < 0):
        raise RuntimeError, "Error: AlbusIonosphere.set_reference_time gave %d"%retval
    ...
    for s in seconds:
        ...
        if(retval < 0):
            raise RuntimeError, "Error: AlbusIonosphere.get_source_AzEl gave %d"%retval
...
"""
    ...
    MJD_end   = jma_aips.get_MJD_hms(year2, month2, day2, hour2, minute2,second2)
