c jma_irisub.F, version number can be found at the end of this comment.
c-----------------------------------------------------------------------        
C 
...
c     
 ...
 901  FORMAT(6X,'CORR.: LIN. APP. BETWEEN HZ=',F5.1,
     &     ' AND HEF=',F5.1)
      T=(XNEHZ-NME)/(HZ-HEF)
      HST=-333.
C     If there is nothing to report here, we are probably out of
C     the allowed altitude range.  Set n_e to zero and bail.
CC      NE_OUT = 0.0
CC      return
      GOTO 4933
...
C      IF((HEIGHT.GT.HNEE).OR.(HEIGHT.LT.HNEA)) GOTO 330
      IF(LAYVER) THEN
         ELEDE=-9.
         IF(IIQU.LT.2) ELEDE=
     &        XEN(HEIGHT,HMF2,NMF2,HME,4,HXL,SCL,AMP)
         ...
         NE_OUT = ELEDE
         print *, 'NE_OUT ', NE_OUT
         IF(NE_OUT.lt.0.0) NE_OUT = 0.0
C     Hey, JMA just wants the density
         print *, 'returning NE_OUT ', NE_OUT
         return
         goto 330
      endif
      ELEDE=XE_1(HEIGHT)
      ...
      IF(NE_OUT.lt.0.0) NE_OUT = 0.0
C     Hey, JMA just wants the density
      print *, 'final return NE_OUT ', NE_OUT
      return
...
C     
 ...
 6092 OARR(23)=XHI
      ...
      OARR(47)=f1pbl
 3330 CONTINUE
      print *, 'at final return NE_OUT =', NE_OUT
      RETURN
      END
c     
c     
      ...
      CHARACTER*256 BPATH
      PARAMETER(BPATH='/home/twillis/albus/libdata/IRI')
...
C     call initialize()
     ...
     &        HEIGHT,NE_OUT)
         ...
         SUM = SUM + NE_OUT * 10.0E3
         print *, HEIGHT, NE_OUT, SUM
      ENDDO
      end
